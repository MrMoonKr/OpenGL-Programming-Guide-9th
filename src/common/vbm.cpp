...
bool VBObject::LoadFromVBM(const char * filename, int vertexIndex, int normalIndex, int texCoord0Index)
{
    ...
    for (i = 0; i < m_header.num_attribs; i++) {
        ...
        glVertexAttribPointer(attribIndex,
                              m_attrib[i].components,
                              m_attrib[i].type,
                              GL_FALSE,
                              0,
                              reinterpret_cast<GLvoid*>(static_cast<uintptr_t>(total_data_size)));
        ...
        if (instances) {
            if (m_header.num_indices)
                glDrawElementsInstanced(GL_TRIANGLES,
                                        ...
                                        GL_UNSIGNED_INT,
                                        reinterpret_cast<const GLvoid*>(static_cast<uintptr_t>(m_frame[frame_index].first * sizeof(GLuint))),
                                        instances);
            else

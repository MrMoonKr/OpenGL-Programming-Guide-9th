...
bool CameraExample::OnInitialize(const AppConfig& config)
{
    ...
    m_viewLocation = glGetUniformLocation(m_program, "u_view");
...
void CameraExample::OnUpdate(float deltaTime)
{
...
void CameraExample::OnDisplay()
{
    glClearColor(0.08f, 0.10f, 0.14f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    glUseProgram(m_program);
    glUniformMatrix4fv(m_viewLocation, 1, GL_FALSE, m_camera.GetViewMatrix());
    glUniformMatrix4fv(m_projectionLocation, 1, GL_FALSE, m_camera.GetProjectionMatrix());
...
void CameraExample::OnResize(int width, int height)
{
...
void CameraExample::OnKey(int key, int scancode, int action, int mods)
{
    (void)scancode;
    (void)mods;
...
void CameraExample::OnMouseButton(int button, int action, int mods)
{
    (void)mods;
...
void CameraExample::OnCursorMove(double x, double y)
{
    if (!m_rightMouseDown)
    {
        return;
...
void CameraExample::ResetCamera()
{
    m_camera.Reset();
    m_camera.SetAspect(m_aspect);
